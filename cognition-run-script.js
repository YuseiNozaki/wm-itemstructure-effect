...
const params = {
  ...
  memoryPositions: 2, // Number of positions to remember
};
...
// Demographic information - Age selection with dropdown
const demographics = {
  type: jsPsychHtmlKeyboardResponse,
  stimulus: function() {
    let html = '<div style="max-width: 600px; margin: 0 auto; padding: 20px; text-align: center;">';
    html += '<h2>年齢を選択してください</h2>';
    html += '<form id="age-form" style="margin: 20px 0;">';
    html += '<div style="margin: 20px 0;">';
    html += '<label for="age-select" style="display: block; margin-bottom: 10px; font-size: 18px;">年齢:</label>';
    html += '<select id="age-select" name="age" required style="padding: 10px; font-size: 16px; width: 200px; border: 2px solid #ccc; border-radius: 5px;">';
    ...
    html += '</div>';
    html += '<div style="margin: 20px 0;">';
    html += '<button type="submit" style="padding: 10px 20px; font-size: 16px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">次へ</button>';
    html += '</div>';
    html += '</form>';
    html += '</div>';
...
const genderSelection = {
  ...
  stimulus: function() {
    let html = '<div style="max-width: 600px; margin: 0 auto; padding: 20px; text-align: center;">';
    html += '<h2>性別を選択してください</h2>';
    html += '<form id="gender-form" style="margin: 20px 0;">';
    html += '<div style="margin: 20px 0;">';
    html += '<label for="gender-select" style="display: block; margin-bottom: 10px; font-size: 18px;">性別:</label>';
    html += '<select id="gender-select" name="gender" required style="padding: 10px; font-size: 16px; width: 200px; border: 2px solid #ccc; border-radius: 5px;">';
    html += '<option value="">選択してください</option>';
...
const practiceExplanation = {
  ...
  stimulus: `
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; text-align: center;">
      <p style="font-size: 18px; margin: 20px 0;">はじめに9回練習を行います</p>
      <p style="font-size: 16px; margin: 20px 0;">
        <strong>下の「次へ」ボタンをクリックして練習を開始してください。</strong>
      </p>
      <div style="margin: 20px 0;">
        <button id="practice-next-btn" style="padding: 12px 24px; font-size: 16px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">次へ</button>
      </div>
    </div>
  `,
  choices: "NO_KEYS",
...
const visualSearchInstructions = {
  ...
  stimulus: function() {
    ...
    if (!isPractice) {
      ...
      progressDisplay = `<p style="font-size: 16px; margin: 10px 0; font-weight: bold; color: #007bff;">${currentMainTrialNumber}回/${totalMainTrials}回</p>`;
    }
    
    return `
      ${progressDisplay}
      <p>次は<strong>${viewCondition === 'window' ? 'ウィンドウ条件' : 
...
const memoryGridRecall = {
  ...
  stimulus: function() {
    ...
    gridHTML += '<p id="selection-info">2つのセルを選択してください。</p>';
    ...
    cells.forEach(cell => {
      cell.addEventListener('click', function() {
        ...
        if (isSelected) {
          ...
          // Select cell only if we haven't reached the limit
...
function generateObjects(area, targetColor, targetShape, structure, viewCondition, isPractice) {
  ...
  if (structure === 'structured') {
    ...
    do {
      targetX = Math.random() * (area.width - objectSize * 2) + objectSize;
      targetY = Math.random() * (area.height - objectSize * 2) + objectSize;
      attempts++;
    ...
    for (let i = 0; i < params.numObjects - 1; i++) {
      ...
      do {
        x = Math.random() * (area.width - objectSize * 2) + objectSize;
        y = Math.random() * (area.height - objectSize * 2) + objectSize;
        ...
        overlapping = distance < objectSize;
...
const memoryFeedback = {
  ...
  stimulus: function() {
    ...
    return `
      <h1>${correct ? '正解!' : '不正解'}</h1>
      <p>下の「次へ」ボタンをクリックして次に進みます。</p>
      <div style="margin: 20px 0;">
        <button id="memory-feedback-next-btn" style="padding: 12px 24px; font-size: 16px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">次へ</button>
      </div>
    `;
  },
  data: {
...
const endScreen = {
  ...
  stimulus: function() {
    ...
    return `
      ...
      <p>実験の正答率: ${searchAccuracy}%</p>
      ${memoryAccuracyText}
      <p><strong>下の「終了」ボタンをクリックして終了します。</strong></p>
      <div style="margin: 20px 0;">
        <button id="end-screen-next-btn" style="padding: 12px 24px; font-size: 16px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">終了</button>
      </div>
    `;
  },
  choices: "NO_KEYS",
...
const practiceCompletionChoice = {
  ...
  stimulus: function() {
    let html = '<div style="max-width: 600px; margin: 0 auto; padding: 20px; text-align: center;">';
    html += '<h1>練習完了</h1>';
    html += '<p style="font-size: 18px; margin: 20px 0;">9回の練習が完了しました。</p>';
...
const wmTaskExplanation = {
  ...
  stimulus: `
    <h1>記憶課題の説明</h1>
    <p>これからは追加で、記憶課題も行います。</p>
    <p>2つの色のついたセルの位置を記憶してください。</p>
    <p><strong>下の「次へ」ボタンをクリックして次に進みます。</strong></p>
    <div style="margin: 20px 0;">
      <button id="wm-task-next-btn" style="padding: 12px 24px; font-size: 16px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">次へ</button>
    </div>
  `,
  choices: "NO_KEYS",
  on_load: function() {
